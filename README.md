...
#### javascript变量污染
 ...
 ##### Js代码分为两个阶段：编译阶段和执行阶段
      1. Js代码的编译阶段会找到所有的声明，并用合适的作用域将它们关联起来，这是词法作用域的核心内容包括变量声明(var a)和函数声明(function a(){})在内的所有声明都会在代码被执行前的编译阶段首先被处理
      2. 过程就好像变量声明和函数声明从他们代码中出现的位置被移动到执行环境的顶部，这个过程就叫做提升只有声明操作会被提升，赋值和逻辑操作会被留在原地等待执行
      3. 变量声明
...
#### javascript预编译原理
https://blog.csdn.net/q1056843325/article/details/52951114
  1. 这个预编译和传统的编译是不一样的（可以理解js预编译为特殊的编译）
   * JavaScript是是解释型语言 就是编译一行 执行一行。传统的编译会经历很多步骤，分词，解析，代码生成等。
  2. JavaScript运行三部曲
   * 语法分析
   * 预编译
   * 解释执行
  ...
  4. js预编译什么时候发生
   * 预编译确确实实是在script代码内执行前发生了
   * 但是它大部分会发生在函数执行前
  5. js预编译实例
  6. 注意
   * 预编译阶段发生变量声明和函数声明，没有初始化行为（赋值），匿名函数不参与预编译只有在解释执行阶段才会进行变量的初始化
  7. 总结 预编译（函数执行前）
   * 创建AO对象（Active Object）
   * 查找函数形参及函数内变量声明，形参名及变量名作为AO对象的属性。值为undefined
   * 实参形参相统一，实参值赋值给形参
   * 查找函数声明，函数名作为AO对象的属性，值为函数引用
  8. 预编译(脚本代码块script执行前) 
...
##### 闭包和高阶函数
   1. 闭包
      ...
      1. 变量的作用域
        * 如果该函数前面没有加上关键字var，这个变量就会成为全局变量   
        * 而对于在函数内用var关键字声明的局部变量来说，只有在该函数内部才能访问到这个变量，在函数外部是访问不到的。
        * 闭包可以延长变量的声明周期
      2. 变量的生命周期
        * 对于全局变量来说，全局变量的声明周期当然式永久的，除非我们主动销毁这个全局变量
        * 而对于在函数内用var关键字声明的局部变量来说，当退出函数时，它们就会随着函数的调用的结束而被销毁。
        * 闭包可以延续变量的生命周期
      ...
      4. 闭包实现命令模式
      5. 闭包与内存管理
        * 可能会引起内存泄漏--如果两个对象之间形成了循环引用，那么这两个对象都无法被回收，但循环引用造成的内存泄漏在本质上也不是闭包造成的。
  2. 高阶函数
      1. 高阶函数是至少满足下面一个条件的函数
       * 函数可以作为参数被传递
       * 函数可以作为返回值输出
      2. 函数作为参数传递
       * 回调函数（异步请求，委托）
       ...
       * getSingle
      4. 高阶函数实现AOP（面向切面编程）
       * AOP(面向切面编程)的主要作用是把一些跟核心业务逻辑模块无关的功能抽离出来，这些跟业务逻辑无关的功能通常包括日志统计，安全控制，异常处理等。把这些功能抽离出来之后，再通过“动态织入”的方式渗入业务逻辑模块中。、
      5. 高阶函数的其他应用
       * currying 又称部分求值。一个currying的函数首先会接受一些参数，接受了这些参数之后，该函数并不会立即求值，而是继续返回另外一个函数，刚才传入的参数在函数形成的闭包中保存起来。待到函数被真正需要求值的时候，之前传入的所有参数都会被一次性求值。
       * 函数节流  将即将被执行的函数用setTimeout延迟一段时间执行。如果该次延迟执行还没有完成，则忽略接下来调用该函数的请求。
...
##### 单例模式
   1. 应用场景
    ...
    2. 使用场景
     * 使用CreateDiv 单例类，它的作用是负责在页面中创建唯一的div节点
    3. 惰性单例
     * 指的是在需要的时候才创建对象的实例。惰性单例是单例模式的重点
     
##### 策略模式
   1. 定义
    * 定义一系列的算法,把它们一个个封装起来，并且使它们可以直接相互替换。
   2. 应用场景1
    1. 未使用策略模式
    2. 使用策略模式
